...
fun AppNavDisplay(modifier: Modifier = Modifier) {
    ...
    NavDisplay(
        ...
        onBack = ::goBack,
        modifier = modifier,
        // 不启用 rememberViewModelStoreNavEntryDecorator：其 entry 级 ViewModelStoreOwner
        // 无法为 AndroidViewModel 提供 Application 工厂，0.5.7 实测安装后启动即闪退
        // （f4-integration 偏差#6，启动路径唯一改动即此 decorator）。会话隔离改用
        // ChatScreen 的 viewModel(key="chat:$sessionId")（偏差#3 的无 decorator 替代修复）。
        entryProvider = { key ->
            when (key) {
