...
| | | **WS connect/hello/heartbeat** | ✅ | 0.5.0（f3a-ws-client） |
| 4. 会话主流程 | 阶段 3 | 会话 CRUD + SSE 状态机 + 审批 + 简单 rewind | ✅ | 0.2.0（PR #3） |
| 5. 知识与管理 | 阶段 4 | **知识库**：搜索/列表/条目/编译状态 | ✅ | 0.4.0（PR #5） |
| | | **管理**：工具/权限/模型列表/监控 | ⚠️ 部分 | 0.3.0（PR #4）仅工具+权限；LLM 列表、监控未做 |
| 6. 设备代理 | 阶段 5 | 前台服务 + RPC + 执行器 | ⚠️ 仅 WS 传输层（0.5.0） | F3 主体未开始 |
...
1. **管理面补全**（阶段 4 后半）：LLM 模型列表、监控 → 计划内待排
2. **设备代理**（阶段 5）：前台服务 + RPC dispatcher + 18 项执行器（先只读后写入）→ F3；WS 传输层已由 f3a-ws-client 交付（0.5.0）
